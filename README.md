...
| `/gender_set` | `设置性别` | 手动设置性别 | 当API无法获取时使用 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `enable_manual_set` | bool | true | 允许手动设置性别 |
| `admin_only` | bool | true | 仅管理员可设置他人性别 |
...
1. **手动设置优先**: 首先检查是否有手动设置的性别
2. **缓存查询**: 检查本地缓存是否有性别信息
3. **API 获取**: 通过 QQ API 获取性别信息
4. **降级处理**: API 返回 "unknown" 时提示手动设置
...
- 手动性别设置：`data/plugin_data/astrbot_plugin_gender_detector/manual_gender.json`
...
1. 确认用户在 QQ 中设置了性别
2. 检查机器人是否有获取用户信息的权限
3. 使用 `/gender_debug` 查看 API 响应
4. 使用 `/gender_set` 手动设置性别
